...
public class QueryHelper implements Serializable{
	 ...
	 * @return
	 */
	public QueryHelper addWhere(String field,String sign, int type){
		if(isBlank(field)) return this;
		if(wheres == null)
			wheres = new ArrayList<Where>();
		Where where = new Where (field,null,type);
		wheres.add(where);
		return this;
	}
	 ...
	 * @return
	 */
	public QueryHelper addWhere(String sql){
		return addWhere(sql, "", Type.CUSTOM);
	}
	...
	public QueryHelper addOrderBy(String type, String ... fields){
		if(isBlank(type) || fields == null) return this;
		for(String f : fields){
